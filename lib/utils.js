...
function isRealMovieTitle(movie) {
  const movieTitle = movie.title;
...
function filterAndCleanMovieTitles(movies) {
  const uniqueMovies = returnUniqueMovies(movies);
  const sortedUniqueMovies = sortMoviesByTitle(uniqueMovies);

  return sortedUniqueMovies.filter((movie) => {
    const cleanedTitle = cleanUpMovieTitles(movie.title);
    return isRealMovieTitle(cleanedTitle);
  });
}
